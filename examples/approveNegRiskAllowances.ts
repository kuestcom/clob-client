...
async function main() {
  ...
  const usdcAllowanceNegRiskAdapter = await usdc.allowance(walletAddress, contractConfig.negRiskAdapter)
  console.log(`usdcAllowanceNegRiskAdapter: ${usdcAllowanceNegRiskAdapter}`)
  const usdcAllowanceNegRiskExchange = await usdc.allowance(walletAddress, contractConfig.negRiskExchange)
