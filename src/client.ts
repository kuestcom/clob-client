...
import { DELETE, del, GET, get, POST, parseDropNotificationParams, post, put } from './http-helpers/index.ts'
...
import { filterSiteScopedMarkets, getSiteMarketScope, hasConfiguredSiteScope } from './site-scope.ts'
...
export class ClobClient {
  ...
  constructor(
    ...
    this.orderBuilder = new OrderBuilder(signer as ClobSigner, chainId, signatureType, funderAddress, getSigner)
    this.tickSizes = {}
  ...
  public async getSamplingSimplifiedMarkets(next_cursor = INITIAL_CURSOR): Promise<PaginationPayload> {
  ...
  public async getTrades(params?: TradeParams, only_first_page = false, next_cursor?: string): Promise<Trade[]> {
    this.canL2Auth()
  ...
  public async getBalanceAllowance(params?: BalanceAllowanceParams): Promise<BalanceAllowanceResponse> {
    this.canL2Auth()
  ...
  public async createOrder(userOrder: UserOrder, options?: Partial<CreateOrderOptions>): Promise<SignedOrder> {
    this.canL1Auth()
  ...
  private async _getSiteScopedMarketPage(endpoint: string, next_cursor = INITIAL_CURSOR): Promise<PaginationPayload> {
    ...
    while (true) {
      ...
      const responseCursor = typeof response?.next_cursor === 'string' ? response.next_cursor : END_CURSOR
  ...
  private async ensureGeoblockAllowed(): Promise<void> {
    ...
    if (this.geoblockStatus === undefined) {
      ...
      const result = await get(`${GEOBLOCK_HOST}/?url=${encodeURIComponent(SITE_CONFIG.site_url)}`)
