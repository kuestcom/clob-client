...
export class ExchangeOrderBuilder {
  ...
  constructor(
    ...
    this.appDomainSeparator = keccak256(
      encodeAbiParameters(
        ...
        [DOMAIN_TYPE_HASH, PROTOCOL_NAME_HASH, PROTOCOL_VERSION_HASH, BigInt(chainId), contractAddress as Address],
      ),
    )
  }
  ...
  async buildOrder({
    ...
    if (signer !== maker) {
      throw new Error('Deposit Wallet orders must use the Deposit Wallet as maker and signer')
    }
