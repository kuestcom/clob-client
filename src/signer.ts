...
interface EthersV6Signer {
  signTypedData(domain: TypedDataDomain, types: TypedDataTypes, value: TypedDataValue): Promise<string>
  getAddress(): Promise<string>
}
...
const isEthersTypedDataSigner = (signer: ClobSigner): signer is EthersSigner =>
  ...
  (hasFunction(signer, 'signTypedData') && hasFunction(signer, 'getAddress') && !isWalletClientSigner(signer))
